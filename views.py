...
from .serializers import (AuthTokenSerializer, EmailUserSerializer, CreateEmailUserSerializer, UpdateUserInfoSerializer,
                         CountrySerializer, StateSerializer, CitySerializer, AppVersionSerializer)
...
EMAILUSER_VALUES = [
        ...
        'country', 'state', 'city', 'ABN', 'timestamp',
        'company_name', 'profile_description',
        'company_logo', 'background_image',
        'address_line_1', 'address_line_2',
        'validated_at',
        'is_seller', 'is_buyer', 'is_terms_conditions_accepted', 'is_superuser',
...
class ObtainAuthToken(OriginalObtain):
    def post(self, request, require_validated=True):
        ...
        notification_user = FCMDevice.objects.filter(user_id=user.id).update(active=False)
        crate_notification_user = FCMDevice.objects.create(registration_id=self.request.data['registration_id'],
                                                           user_id=user.id, type=self.request.data['type'])
        return Response({
            'result' : 'success',
            'email' : user.email,
...
class EmailUserViewSet(viewsets.ModelViewSet):
    ...
    def check_registered(self, request):
        if 'user_id' in request.data:
            ...
            user = EmailUser.objects.filter(id=request.data['user_id']).values(*EMAILUSER_VALUES)

            notifications = Notifications.objects.filter(user_id=request.user.id).filter(is_read=False)
            try:
                user_data_dir['company_logo'] = request.build_absolute_uri(user_details.company_logo.url)
            except Exception as e:
                user_data_dir['company_logo'] = None

            try:
                user_data_dir['background_image'] = request.build_absolute_uri(user_details.background_image.url)
            except Exception as e:
                user_data_dir['background_image'] = None

            try:
                user_data_dir['user_image'] = request.build_absolute_uri(user_details.image.url)
            except Exception as e:
            ...
            user_data_dir['cartCount'] = Cart.objects.filter(buyer_id=request.data['user_id']).filter(
                                                             coffee__status=True).filter(coffee__is_sold_out=False).count()
            user_data_dir['notificationsCount'] = notifications.count()
            user_data_dir['messageCount'] = Message.objects.filter(received_to=request.user.id).filter(is_read=False).count()
            user_data_dir['is_social'] = request.user.is_social
            user_data_dir['is_seller_approved'] = request.user.is_seller_approved
...
class UpdateEmailUserViewSet(viewsets.ModelViewSet):
    ...
    def updates(self, request, *args, **kwargs):
        ...
        if 'new_state' in request.data:
            if request.data['new_state'] != "None" and request.data['new_state'] != '':
                state = State.objects.create(
                    name=request.data['new_state'],
                    country_id=request.data['country']
                )
            if request.data['new_city'] != "None" and request.data['new_city'] != '':
    ...
    def send_invoice(self, mapping_id):
        ...
        buyer_delivery_details = OrderPlaced.objects.filter(order_placed_mapping_id=mapping_id)[0].shipping

        orders = OrderPlaced.objects.filter(order_placed_mapping_id=mapping_id).annotate(
            coffee_name=F('coffee__name'),
            seller_email=F('seller__email'),
            grind=F('selected_grind__grind')
        ).values('coffee_name', 'weight', 'qty', 'price', 'seller_email', 'grind')

        subtotal = orders.aggregate(Sum('price'))

        d = { 'orders': orders,
              'subtotal' : subtotal['price__sum'],
              'total': orderPlaced_mapping.price,
              'coupon_code' : orderPlaced_mapping.coupon_code,
              'coupon_discount_amount' : orderPlaced_mapping.coupon_discount_amount,
              'invoice_no': mapping_id,
              'buyer_name' : self.request.user.first_name + ' ' + self.request.user.last_name,
              'address' : buyer_delivery_details.address_line_1,
              'postcode': buyer_delivery_details.postal_code,
              'city': buyer_delivery_details.city.name,
              'state': buyer_delivery_details.state.name,
              'country': buyer_delivery_details.country.name,
              'contact_no' : buyer_delivery_details.phone_number,
              'buyer_email' : self.request.user.email,
              'date_of_invoice' : OrderPlacedMappig.objects.get(id=mapping_id).timestamp.date(),
              'payment_symbol' : self.request.user.country.currency_symbol}
        ...
        text_content = plaintext.render(d)
        html_content = htmly.render(d)
        msg = EmailMultiAlternatives(subject, text_content, from_email, [to], bcc=[settings.ADMINS])
        msg.attach_alternative(html_content, "text/html")
        msg.send()

        sellers = orders.values_list('seller_id', 'seller_email').distinct()
        for seller in sellers:
            order1 = OrderPlaced.objects.filter(order_placed_mapping_id=mapping_id, seller=seller[0]).annotate(
                coffee_name=F('coffee__name'),
                seller_email=F('seller__email'),
                grind=F('selected_grind__grind')
            ).values('coffee_name', 'weight', 'qty', 'price', 'seller_email', 'grind')
            ...
            subject, from_email, to = 'New Order', settings.DEFAULT_FROM_EMAIL, seller[1]
            text_content = plaintext.render(d)
            html_content = htmly.render(d)
            msg = EmailMultiAlternatives(subject, text_content, from_email, [to], bcc=[settings.ADMINS])
            msg.attach_alternative(html_content, "text/html")
    ...
    def usd_cof_value(self, request, *args, **kwargs):
        ...
        if not isinstance(usd, float):
            subject, from_email, to = 'COF Scraping code Errors', settings.DEFAULT_FROM_EMAIL, ['vijendra@tersesoft.com', 'khwab@tersesoft.com']
            msg = EmailMultiAlternatives(subject, 'sjdklfjsj', from_email, to)
            msg.attach_alternative("Your COF scraping code is not working. Please check it...", "text/html")
            msg.send()

            file=open("COF_price.txt", "r")
            usd =file.read()
            destination_address = settings.COF_DESTINATION_ADDRESS
            return Response(data={'cof':float(usd), 'destination_address': destination_address})
...
#previous code----->
        ...
        # usd = float(name_box.strip())
            ...
            file.close
            destination_address = settings.COF_DESTINATION_ADDRESS
            return Response(data={'cof':usd, 'destination_address': destination_address})

    def order_placed_handled(self):
        user = self.request.user
        cart_coffee = Cart.objects.filter(coffee_id__in=self.request.data['coffee_ids']).filter(buyer_id=user.id).delete()
        ...
        if 'cof_payment' in self.request.data:
            if 'coupon_code' in self.request.data:
                order_placed_mapping = OrderPlacedMappig.objects.create(
                    ...
                    coupon_discount_amount = self.request.data['coupon_discount_amount'],
            ...
            paymet = config.transaction.sale({
                ...
                "merchant_account_id": str(self.request.user.country.braintree_merchanr_id),
                "payment_method_nonce": str(self.request.data['payment_method_nonce'])
                })
        ...
        for coffee_details in self.request.data['coffee_object']:
            ...
            if 'cof_payment' in self.request.data:
                orderPlaced_coffee = OrderPlaced.objects.create(
                    ...
                    selected_grind=CoffeeGrindTypes.objects.get(id=coffee_details['selected_grind']),
        ...
        if 'cof_payment' in self.request.data:
            if len(firstOrderPlaced) == 0:
                device.send_message(title="TurtleBeans", body="Congratulations you have just placed your first product using COF. Your payment is under process.")
                create_notification = Notifications.objects.create(
                user_id=user.id,
                Notification_image_id=NotificationsImages.objects.get(type='first_order').id,
                title='TurtleBeans',
                body='Congratulations you just brought your first product.',
                type='first_order')
        else:
            if len(firstOrderPlaced) == 0:
                device.send_message(title="TurtleBeans", body="Congratulations you just brought your first product.")
                create_notification = Notifications.objects.create(
                user_id=user.id,
                Notification_image_id=NotificationsImages.objects.get(type='first_order').id,
                title='TurtleBeans',
        ...
        if 'cof_payment' in self.request.data:
            device.send_message(title="TurtleBeans", body="Thanks for purchasing with Turtlebeans! your fresh roast will be with you soon. Please check your email for more information. Your payment is under process.")
            create_notification = Notifications.objects.create(
            user_id=user.id,
            Notification_image_id=NotificationsImages.objects.get(type='order_placed').id,
            title='TurtleBeans',
            body="Thanks for purchasing with Turtlebeans! your fresh roast will be with you soon. Please check your email for more information. Your payment is under process.",
            type='order_placed')
            ...
            device.send_message(title="TurtleBeans", body="Thanks for purchasing with Turtlebeans! your fresh roast will be with you soon. Please check your email for more information.")
            create_notification = Notifications.objects.create(
            user_id=user.id,
            Notification_image_id=NotificationsImages.objects.get(type='order_placed').id,
            title='TurtleBeans',
            body="Thanks for purchasing with Turtlebeans! your fresh roast will be with you soon. Please check your email for more information.",
            type='order_placed')

            device = FCMDevice.objects.filter(user_id__in=seller_ids).filter(active=True)
            device.send_message(title="TurtleBeans", body="It's time to roast! Order No: "+ str(order_placed_mapping.id)+" has just been made. Please check your email for more information.")
            create_notification = Notifications.objects.create(
            user_id=coffee.seller.id,
            Notification_image_id=NotificationsImages.objects.get(type='order_placed').id,
            title='TurtleBeans',
            body="It's time to roast! Order No: "+ str(order_placed_mapping.id)+" has just been made. Please check your email for more information.",
            type='order_placed')
...
class CityViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if 'country_id' in self.request.query_params and 'state_id' in self.request.query_params:
            return q.filter(country=self.request.query_params['country_id']).filter(state=self.request.query_params['state_id'])
        return q.all()
    ...
    def postcode_details(self, request):
        ...
        if location_dir['city']:
            location_dir['state'] = State.objects.filter(id=location_dir['city'][0]['state_id']).values()
            location_dir['Country'] = Country.objects.filter(id=location_dir['state'][0]['country_id']).values()
...
class ChangePassword(views.APIView):
    ...
    def post(self, *args, **kwargs):
        ...
        if user is None:
            return Response(
                data={'result':False, 'message':"Current password is not matched!"},
                status=200)
        if user is not None and (pw1 and pw2):
            user.set_password(pw2)
            user.save()
            return Response(data={'result':True, 'message':'Password updated successfully.'},status=status.HTTP_202_ACCEPTED)
        return Response(
            data={'result':False, 'message':"Missing/Not Matching Passwords"},
            status=200)
...
def cof_send_invoice(mapping_id):
    ...
    if orderPlaced_mapping.attachment != '':
        ...
        orders = OrderPlaced.objects.filter(order_placed_mapping_id=mapping_id).annotate(
            ...
            seller_email=F('seller__email'),
            grind=F('selected_grind__grind')
        ).values('coffee_name', 'weight', 'qty', 'price', 'seller_email', 'seller', 'grind')
